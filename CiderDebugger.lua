...
local CiderRunMode = {};CiderRunMode.runmode = 'RUN';CiderRunMode.assertImage = true;CiderRunMode.userdir = "/Users/yogome/Library/Application Support/luaglider2/dev";local SOCKET_PORT=39454;local GLIDER_MAIN_FOLDER= "/Users/yogome/github/emogoy";local useNativePrint= false;local snapshotInterval= -1;local snapshotInterval= -1;local fileFilters= {"CiderDebugger.lua",};local startupMode= "require";local function shouldDebug()
        ...
        while(numTries<maxTries) do
            ...
            while(line) do  
                if(line and line:len()>3) then                  
                    line = json.decode(line)
                    if(line.type=="s") then
                        rawset(_G, "print", debugPrint)
                        rawset(_G, "error", debugError)
